...
# 2. Main Application View (after login)
    ...
    if st.session_state.main_app_page == "Chat":
        ...
        def create_new_dataset(name: str, description: Optional[str] = None):
            ...
            try:
                ...
                if response.status_code == 201: # Created
                    ...
                    # get_user_datasets() # まだ get_user_datasets は定義していないのでコメントアウト
                    return True
                else:
        ...
        # st.write("アップロードしたドキュメントはRAG検索対象になります。TXT/MD/PDF対応。") # 元のコードはコメントアウト

        # --- Vector Store Strategy Selection ---
        # st.subheader("Vector Store Processing Strategies") # 元のコードはコメントアウト
