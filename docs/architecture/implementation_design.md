...
- **概要**: Agentic Context Engineering (ACE) のコンセプトに基づき、システムがユーザーとの対話から学び、知識を継続的に進化させていく自己改善型の検索戦略です。過去の対話から生成された「進化したコンテキスト」を、将来の検索に活用します。本戦略を駆動するプロンプトのカスタマイズ方法については、[こちらのガイド](./../guides/customizing_ace.md)を参照してください。
- **動作原理**:
