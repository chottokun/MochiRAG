...
-   `cli.py`: 管理者向けのコマンドラインインターフェース。

### 2.3. データ管理モデル

MochiRAGは、2種類のデータセット（ベクトルデータベース）を扱います。

-   **パーソナルデータベース (Personal Database):**
    -   各ユーザーに紐づくプライベートなデータベースです。
    -   ユーザーはUIを通じて、ドキュメントのアップロード、データセットの作成・削除が可能です。
    -   データセットのメタデータ（名前、説明、所有者情報など）は、`User DB (SQLite)`に格納されます。
    -   ベクトルストア内のコレクション名は、`user_{user_id}`のようにユーザーIDに基づいています。

-   **共有データベース (Shared Database):**
    -   全ユーザーが共通で利用できる読み取り専用のデータベースです。
    -   管理者が、`cli.py`の`create-shared-db`コマンドを使用して作成・登録します。
    -   利用可能な共有データベースの情報は、プロジェクトルートの`shared_dbs.json`ファイルで定義されます。このファイルは、UIに表示される名前と、ベクトルストア内のコレクション名をマッピングします。
    -   ベクトルストア内のコレクション名は、`shared_{db_name}`のような命名規則に従います。
