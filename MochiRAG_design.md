...
* **参照番号付与**: LLMの回答自体に参照番号（例: `[1]`, `[2]`）を直接埋め込むことは現状の実装では行っていませんが、将来的には検討可能です。
* **参照ドキュメント情報の提供**: バックエンドAPI (`/chat/query/`) は、LLMが生成した回答文字列に加えて、その回答の根拠となった可能性のある検索されたドキュメント（チャンク）のリストを返します。このリストには、各ドキュメントのメタデータ（ファイル名、ページ番号など）および内容の断片が含まれます。
* **UIでの出典リスト表示**: フロントエンドのチャットUIでは、AIの回答の下に、取得された参照ドキュメントのリストを表示します。
    *   この表示は、ユーザーがサイドバーのチェックボックスで表示/非表示を切り替えることができます。
    *   表示される際には、折りたたみ可能なセクション（例: Streamlitの `st.expander`）内に、各参照元ドキュメントの情報（ファイル名やID、ページ番号、内容の先頭部分など）がリスト形式で提示されます。
    *   （将来的には、各出典をクリックすると対応ドキュメントの該当箇所にジャンプするビューア機能も検討できます。）
